 ...
 * декомпилированного Terraria 1.4.4.1 (br4dnblehh/terraria-source-code).
 *
...
const source = fs.readFileSync(path.join(SRC, "vanilla/ItemDropDatabase.cs"), "utf8");
 ...
 * Source: Terraria 1.4.4.1 decompiled ItemDropDatabase.cs (br4dnblehh/terraria-source-code).
 * npc: { "<npcId>": [[itemId, chanceDenominator, min, max, flags], ...] }
 * flags: 1 master, 2 boss bag / expert, 4 conditional, 8 one-of-options
 */
window.CALAMITY_VANILLA_DROPS = { format: 1, npc: ${JSON.stringify(output)} };
`);
console.log(`Обработано регистраций: ${statements}; NPC с дропом: ${drops.size}; пар NPC→предмет: ${pairs}`);
